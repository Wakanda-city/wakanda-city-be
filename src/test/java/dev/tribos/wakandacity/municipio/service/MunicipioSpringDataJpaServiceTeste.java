...
public class MunicipioSpringDataJpaServiceTeste {
	...
	void setUp() {
		...
		this.municipio = Municipio.builder().codigo(11).nome("suzano").uf("sp").build();
	}
	...
	void deveListaDeCidadesPeloNomeO() {
		...
		List<Municipio> cidadeRetornada = municipioSpringDataJPAService
				.buscaMunicipioService(municipio.getNome().toString());

		assertEquals(cidadeRetornada.get(0), municipio);
		Mockito.verify(municipioRepository).buscaMunicipio(municipio.getNome());
